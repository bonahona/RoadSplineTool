...
namespace Fyrvall.BonaRoadTool
{
    ...
    public class BezierCurve
    {
        ...
        public static float BezierCurveLength(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, int segmentCount)
        {
            var result = 0f;

            var increment = 1f / segmentCount;
            for (int i = 0; i < segmentCount; i++) {
                result += Vector3.Distance(CubicCurve(p0, p1, p2, p3, increment * i), CubicCurve(p0, p1, p2, p3, increment * (i + 1)));
            }
